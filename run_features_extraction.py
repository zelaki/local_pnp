...
def main():
    ...
    outpath = f"{exp_path_root}/{exp_config.config.experiment_name}_{seed}"
    ...
    def save_feature_maps_callback(i):
        if opt.save_all_features:
            for b in unet_model.middle_block:
                if "ResBlock" in str(type(b)):
                    save_feature_map(b.in_layers_features, f"middle_in_layers_features_time_{i}")
                    save_feature_map(b.out_layers_features, f"middle_out_layers_features_time_{i}")
