...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape, negative_conditioning=None,
                      ...
                      negative_prompt_alpha=1.0, negative_prompt_schedule='constant', timestep=None, asyrp=False):
        ...
        for i, step in enumerate(iterator):
            ...
            if timestep is not None:
                ...
                if (injected_features is not None and len(injected_features) > 0):
                    injected_features_i = injected_features[i]
                else:
                    injected_features_i = None
    ...
    def p_sample_ddim(self, x, c, t, index, negative_conditioning=None,
                      ...
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,
                      injected_features=None, negative_prompt_alpha=1.0, asyrp=False
                      ):
...
############################## Im trying to implement Asyrp #############################################
        if asyrp:
            if negative_conditioning is not None:
                ...
                c_in = torch.cat([nc, uc])
                e_t_negative_or, e_t_uncond_or = self.model.apply_model(x_in,
                                                        t_in,
                                                        c_in,
                                                        injected_features=None,
                                                        ).chunk(2)
                ...
                c_in = torch.cat([uc, c])
                e_t_uncond_or, e_t_or = self.model.apply_model(x_in,
                                                        t_in,
                                                        c_in,
                                                        injected_features=None,
                                                        ).chunk(2)

                e_t_tilde_or = negative_prompt_alpha * e_t_uncond_or + (1 - negative_prompt_alpha) * e_t_negative_or
                e_t_or = e_t_tilde_or + unconditional_guidance_scale * (e_t_or - e_t_tilde_or)
            else:
                x_in = torch.cat([x] * 2)
                t_in = torch.cat([t] * 2)
                c_in = torch.cat([unconditional_conditioning, c])
                e_t_uncond_or, e_t_or = self.model.apply_model(x_in, t_in, c_in).chunk(2)
                e_t_or = e_t_uncond_or + unconditional_guidance_scale * (e_t_or - e_t_uncond_or) if unconditional_guidance_scale!=1 else e_t_or

            if score_corrector is not None:
                assert self.model.parameterization == "eps"
                e_t_or = score_corrector.modify_score(self.model, e_t_or, x, t, c, **corrector_kwargs)
############################## Im trying to implement Asyrp #############################################
...
#        direction pointing to x_t
        if asyrp:
            dir_xt = (1. - a_prev - sigma_t**2).sqrt() * e_t_or
        else:
